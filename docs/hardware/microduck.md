...
OpenCastor does not use either port. Its own path is robotd's unix socket over
an SSH forward, which costs an SSH key and the `robot` group and is real
protection. But putting the duck on your Wi-Fi so OpenCastor can reach it puts
`mediad` on your Wi-Fi too, and this guide inherits that fact.
