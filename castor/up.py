...
def run_up(*, home: Path, name: str | None = None, archetype: str | None = None,
    ...
    if bearers.exists():
        ...
        _say("bearers: reused existing", started)
    else:
        ...
        # gateway KeyErrors on anything else — caught live when the first
        # scratch robot's gateway crash-looped on exactly this. And the file
        # doubles as gateway config: the `actuator` section names which entry
        # point the gateway constructs. rc-car with an empty config is the
        # SIMULATED-wheels default for every archetype — real PWM is a
        # deliberate later flip in gateway-policy.env, never a setup default.
        bearers.write_text(
            ...
            "  config: {}\n")
        bearers.chmod(0o600)
        _say("bearers: generated (actuate + read)", started)
    ...
    gateway_bin = str(sibling) if sibling.exists() else (
        shutil.which("robot-md-gateway") or str(sibling))
