...
Because the hook and the runtime now append to one file, they also share one
lock. The runtime's own append used to be guarded by an in-process lock only,
which a separate bash process cannot see, so a read of the last line from one
writer could straddle an append from the other and produce a `prev_hash`
pointing at a line that was no longer last: a break the verifier would report
as tampering when nothing had been tampered with. `castor.audit` now takes the
same advisory lock file `audit_log.sh` takes, around the read and the append
together. Where the platform has no `fcntl` the write still goes through.
The generated hook also resolves its log path when it runs rather than when it
was written, so a script generated by a process with no `ROBOT_HOME` cannot
keep pointing at a different file than the runtime it serves.
